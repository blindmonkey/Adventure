 ...
 *
 * There is a scheme interpreter built into the game.
 * The way to use it is you have to use the computer on the homework
 * (use computer homework).  Then you type out the scheme program specified.
 * The commands available are displayed with "(help)".
 * When you exit, your code should be automatically saved.
 * The interpreter supports multiple lines.
 * Also, if you type in "(test)", it tests your program against written tests.
 * This shouldn't be used, in general, as you should find the grader every time
 * you want to check your homework.
 * 
 * There is also a scheme interpreter available to play around with, if you simply
 * run the Scheme class.
 * 
 * I realize the interpreter is a bit inefficient in some parts and a bit incomplete in others.
 * Also, there wasn't much time to comment, revise and refactor the code, as
 * this was quite an ambitious project.
 * 
 * Sincerely,
 * Sergey Grabkovsky
 */
...
public class Adventure {
    ...
    public static void createWorld (String myName) {
        ...
        Thing.create("lake-hall-dungeion", lake);
         ...
         * (define (add-list lon)
         * (cond [(empty? lon) 0]
         *       [else (+ (first lon) (add-list (rest lon)))]))
         */
        UnfinishedHomework.create("hw-1",
                        World.pickRandom(World.rooms()),
                        "For this assignment, please write a function that adds up a list of numbers in Scheme with the following contract:\n" +
                        "add-list : [ListOf Number] -> Number\n",
                        "(and (equals? (add-list (list 1 2 3)) 6)" +
        "(equals? (add-list ()) 0) (equals? (add-list (list 74 24 123 535)) 756))");
        /*
         * Solution to hw-2
         * (define (distance x1 y1 x2 y2)
         * (sqrt (+ (^ (- x2 x1) 2) (^ (- y2 y1) 2))))
         */
        UnfinishedHomework.create("hw-2",
                        World.pickRandom(World.rooms()),
                        "For this assignment, please write a distance function in Scheme with the following contract:\n" +
                        "distance : Number Number Number Number -> Number\n",
                        "(and (equals? (distance 0 0 1 1) (sqrt 2))" +
                        "(equals? (distance 0 1 0 4) 3)" +
        "(equals? (distance -1 -1 1 1) (sqrt 8)))");
        /*
         * Solution to hw-3
         * (define (a-h lon acc n)
         * (cond [(empty? lon) (/ acc n)]
         *       [else (a-h (rest lon) (+ acc (first lon)) (+ 1 n))]))
         * (define (average lon)
         * (cond [(empty? lon) 0]
         *       [else (a-h (rest lon) (first lon) 1)]))
         */
        UnfinishedHomework.create("hw-3",
                        World.me().location(), //World.pickRandom(World.rooms()),
                        "For this assignment, please write function in Scheme with the following contract:\n" +
                        "average : [ListOf Number] -> Number\n",
                        "(and (equals? (average (list 1 2 3 4)) 2.5)" +
                        "(equals? (average (list 2 4)) 3)" +
                        "(equals? (average (list)) 0)" +
        "(equals? (average (list 1)) 1))");
        /*
         * Solution to hw-4
         * (define (factorial n)
         * (cond [(<= n 0) 1]
         ...
         */
        UnfinishedHomework.create("hw-4",
                        World.pickRandom(World.rooms()),
                        "For this assignment, please write factorial in Scheme that has the following contract:\n" +
                        "factorial : Number -> Number\n" +
                        "This function will calculate the factorial of a given number.",
                        "(and (equals? (factorial 5) 120) (equals? (factorial 4) 24) " +
        "(equals? (factorial 3) 6) (equals? (factorial 0) 1))");//World.pickRandom(World.rooms()));
        ...
        /*
         * Debugging purposes 
         * sets the game to put these in wvhlab
         * need to change a hw assignment to wvhlab
         * and the avator to wvhlab
         *
        Grader.create("jerky", wvhlab, 1, 1);
        Professor.create("ass", wvhlab, 1, 1);
         */
    ...
    public static void createVocabulary () {
        World.registerVerb(DualUse.create());
